...
Norconex is an IT company from Gatineau (Quebec) specialized in the enterprise search. Norconex also propose a very nice open source Web Crawler kown as _HTTP Collector_. But, Norconex crawler is, before that, a generic pluggable crawl engine and we can see its structure on the image below.  
...
This is certainly more interesting to work with winery surfaces instead of wineries count to really see what kind of grapes are the most cultivated. Also, with surfaces, it is possible to know what winery is the greatest actor of a region.
It is also possible to localize wineries and build a map to see where they are (and also compute a density). It is possible to improve the varietal nodes with color or origin data, by crossing these data with another crawl based on another web site wich contains these kind of information. As you can see, this sample could be extended to become a real data reference about Californian wines.



== Conclusion

The combination of Norconex and Neo4j leads to strong potential. Because with the use of Neo4j Committer, we are able to grab linked data instead of the traditional tabular view. However, Norconex is also able to run many committers simultaneously, and we could imagine to store related data in Neo4j and content data (like text) in Elastic Search in a single pass for example.

== Resources

All images in this article are built from Neo4j Bloom vizualisation tool: link:ex-url.adoc[https://neo4j.com/bloom/]
The Norconex configuration is based on Neo4j Committer v2:  link:ex-url.adoc[https://www.norconex.com/collectors/committer-neo4j/]
